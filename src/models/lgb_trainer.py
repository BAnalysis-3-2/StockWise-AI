...
def forecast_lgb(user_path, min_rows=90):
    train_path = os.path.join(user_path, "training_features.csv")
    output_path = os.path.join(user_path, "forecasts_lgb_7d.csv")
    model_dir = os.path.join(user_path, "models")
    os.makedirs(model_dir, exist_ok=True)

    print(f"🚀 Training LightGBM per-SKU for: {user_path}")
    ...
    for sku in tqdm(df['sku'].unique()):
        ...
        joblib.dump(model, os.path.join(model_dir, f"lgb_model_{sku}.pkl"))
